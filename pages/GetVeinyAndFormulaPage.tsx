...
const DEFAULT_REVIEW_CARDS: FeaturableReview[] = [
  {
    id: '1',
    title: 'Full pump. Full flavor.',
    body: 'The Veiny Ahh drink gave me an insane pump without the jitters. Highly recommend for anyone who loves vascularity.',
    author: 'Jess, verified buyer',
    rating: 5,
  },
  {
    id: '2',
    title: 'No stimulant, all pump.',
    body: 'This drink is wild. Great taste and performance without the crash. I’m hooked.',
    author: 'Tyler B.',
    rating: 5,
  },
];

const REVIEWS_WIDGET_API = 'https://featurable.com/api/v2/widgets/f8fda964-a81a-4e42-ba42-dd6a84e8493f';
const FEATURABLE_WIDGET_DIV_ID = 'featurable-f8fda964-a81a-4e42-ba42-dd6a84e8493f';
const FEATURABLE_WIDGET_SCRIPT_ID = 'featurable-widget-embed-script-f8fda964-a81a-4e42-ba42-dd6a84e8493f';

const normalizePayload = (raw: any) => {
  if (!raw) return null;
  let payload = raw;
  if (payload.data) payload = payload.data;
  if (payload.widget) payload = payload.widget;
  if (payload.attributes) payload = payload.attributes;
  return payload;
};

const parseReviewsFromPayload = (payload: any) => {
  const normalized = normalizePayload(payload);
  const reviewArray = Array.isArray(normalized?.reviews)
    ? normalized.reviews
    : Array.isArray(normalized?.data?.reviews)
    ? normalized.data.reviews
    : [];

  return reviewArray.map((item: any, index: number) => ({
    id: item.id?.toString() ?? item.review_id?.toString() ?? String(index),
    title: item.title ?? item.summary ?? undefined,
    body: item.body ?? item.text ?? item.review_text ?? item.description ?? '',
    text: item.body ?? item.text ?? item.review_text ?? item.description ?? '',
    rating: Number(item.rating ?? item.stars ?? item.score ?? item.rating_value ?? 0),
    author: item.author ?? item.name ?? item.reviewer?.name ?? item.source ?? 'Anonymous',
    source: item.source ?? item.platform ?? undefined,
    date: item.date ?? item.created_at ?? item.review_date ?? undefined,
  }));
};
...
const ReviewsSection: React.FC = () => {
  ...
  const [reviews, setReviews] = useState<FeaturableReview[]>(DEFAULT_REVIEW_CARDS);
  const [selectedReviewIndex, setSelectedReviewIndex] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [fetchError, setFetchError] = useState('');
  const [writeReviewUrl, setWriteReviewUrl] = useState('https://featurable.com');

  useEffect(() => {
    let canceled = false;
    setIsLoading(true);

    fetch(REVIEWS_WIDGET_API)
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        ...
        const maybeRating = getNumericValue(
          payload?.google_rating ?? payload?.average_rating ?? payload?.rating ?? payload?.stars ?? payload?.rating_value
        );
        const maybeReviewCount = getNumericValue(
          payload?.google_review_count ?? payload?.review_count ?? payload?.reviews_count ?? payload?.total_reviews ?? payload?.count ?? (payload?.reviews?.length ?? undefined)
        );
        const extractedReviews = parseReviewsFromPayload(payload);

        setRating(maybeRating ?? 0);
        setReviewCount(maybeReviewCount ?? extractedReviews.length ?? 0);
        setReviews(extractedReviews.length ? extractedReviews : DEFAULT_REVIEW_CARDS);
        setWriteReviewUrl(
          payload?.write_review_url ?? payload?.review_url ?? payload?.review_link ?? 'https://featurable.com'
        );
      })
      .catch((error) => {
        if (canceled) return;
  ...
  useEffect(() => {
    if (!expanded) return;
    if (document.getElementById(FEATURABLE_WIDGET_SCRIPT_ID)) return;

    const script = document.createElement('script');
    script.id = FEATURABLE_WIDGET_SCRIPT_ID;
  ...
  const currentReview = reviews[selectedReviewIndex];
  const safeRating = Number.isFinite(rating ?? currentReview?.rating ?? 0) ? rating ?? currentReview?.rating ?? 0 : 0;
  const safeReviewCount = reviewCount ?? reviews.length;

  const handlePrev = () => setSelectedReviewIndex((prev) => (prev <= 0 ? reviews.length - 1 : prev - 1));
  const handleNext = () => setSelectedReviewIndex((prev) => (prev >= reviews.length - 1 ? 0 : prev + 1));

  return (
    <section className="mt-10 md:mt-14">
      <div className="max-w-5xl mx-auto px-4 md:px-0">
        <div className="overflow-hidden rounded-[2rem] border-2 border-white/10 bg-zinc-950/90">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white/5 px-6 py-5 md:px-8 md:py-6">
            <div className="space-y-1">
              <div className="text-3xl md:text-4xl font-black uppercase tracking-[0.2em] text-white">Google Reviews</div>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-white/70">
                <span className="font-black text-5xl md:text-6xl text-blood-red">{isLoading ? '—' : safeRating.toFixed(1)}</span>
                <span className="uppercase text-sm tracking-[0.25em]">{isLoading ? 'Loading rating' : `${safeReviewCount ?? 0} reviews`}</span>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setExpanded((current) => !current)}
              className="inline-flex items-center justify-center rounded-full border-2 border-blood-red bg-blood-red px-5 py-3 text-sm md:text-base font-black uppercase tracking-[0.2em] text-white transition hover:bg-white hover:text-black"
            >
              {expanded ? 'Collapse reviews' : 'Show reviews'}
            </button>
          </div>

          {expanded && (
            <div className="border-t border-white/10 px-6 py-8 md:px-8 md:py-10">
              <div className="grid gap-8 lg:grid-cols-[1.4fr_0.9fr]">
                <div className="bg-black/70 rounded-[2rem] border border-white/10 p-8">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-5xl md:text-6xl font-black text-white">{safeRating.toFixed(1)}</div>
                      <div className="mt-1 uppercase text-xs tracking-[0.3em] text-white/60">Google rating</div>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl md:text-4xl font-black text-white">{safeReviewCount}</div>
                      <div className="uppercase text-xs tracking-[0.3em] text-white/60">reviews</div>
                    </div>
                  </div>

                  <div className="mt-8 rounded-[1.75rem] border border-white/10 bg-zinc-950/90 p-6">
                    <div className="flex items-center justify-between gap-4">
                      <button
                        type="button"
                        onClick={handlePrev}
                      ...
                      <div className="flex-1 px-4">
                        <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.25em] text-blood-red">
                          {Array.from({ length: 5 }, (_, idx) => (
                            <span key={idx}>{idx < Math.round(safeRating) ? '★' : '☆'}</span>
                          ))}
                        </div>
                        <p className="mt-4 text-white text-base leading-relaxed">"{currentReview?.body ?? currentReview?.text ?? 'The review content is not available yet.'}"</p>
                        <div className="mt-4 text-sm text-white/70">
                          {currentReview?.author || currentReview?.source || 'Anonymous'}
                  ...
                  <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
                    <a
                      ...
                      className="inline-flex items-center justify-center rounded-full border-2 border-blood-red bg-blood-red px-6 py-3 text-sm md:text-base font-black uppercase tracking-[0.2em] text-white transition hover:bg-white hover:text-black"
                    >
                      Write a review
                    </a>
                    {fetchError && (
                      <p className="text-sm text-rose-400">Unable to load live review details. Showing sample reviews instead.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                    <div id={FEATURABLE_WIDGET_DIV_ID} data-featurable-async className="min-h-[240px]"></div>
                  </div>
                  <p className="text-xs uppercase tracking-[0.3em] text-white/50">Powered by Featurable</p>
                </div>
              </div>
            </div>
          )}
